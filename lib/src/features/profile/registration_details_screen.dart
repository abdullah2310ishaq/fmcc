...
class _RegistrationDetailsScreenState extends State<RegistrationDetailsScreen> {
            ...
            child: Column(
              ...
              children: [
                ...
                FilledButton(
                  ...
                  child: Text(_saving ? 'Saving...' : 'Continue • جاری رکھیں'),
                ),
              ],
            ),
          ),
  ...
  Future<void> _loadProvinces(SessionController controller) async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _provinces = items.map((p) => (id: p.id, name: p.name)).toList(growable: false);
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _loadDistricts(SessionController controller) async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _districts = items.map((d) => (id: d.id, name: d.name)).toList(growable: false);
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _loadTehsils(SessionController controller) async {
    ...
    try {
      final items = await controller.fetchTehsils(provinceId: pid, districtId: did);
      if (!mounted) return;
      setState(() {
        _tehsils = items.map((t) => (id: t.id, name: t.name)).toList(growable: false);
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    return Text.rich(
      TextSpan(
        ...
        children: [
          TextSpan(
            // Force RTL for Urdu part while keeping a single line.
            text: ' - \u200F$ur',
            style: TextStyle(
              fontSize: 12.sp,
              color: AppColors.textSecondary,
